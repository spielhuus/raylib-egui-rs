...
#[repr(C)]
#[derive(Debug, Copy, Clone, Default, Eq, PartialEq)] // TODO: added Eq and PartialEq see below
pub struct Color {
    pub r: u8,
    pub g: u8,
    pub b: u8,
...
impl From<i32> for Color {
    fn from(col: i32) -> Color {
        let r: u8 = ((col >> 24) & 0xff) as u8;
        let g: u8 = ((col >> 16) & 0xff) as u8;
        let b: u8 = ((col >> 8) & 0xff) as u8;
        let a: u8 = (col & 0xff) as u8;
...
impl Color {
    ...
    pub fn color_to_int(&self) -> i32 {
        unsafe { crate::ffi::ColorToInt(self.into()) }
    }

    /// Returns color normalized as float [0..1]
    #[inline]
    pub fn color_normalize(&self) -> Vector4 {
        unsafe { crate::ffi::ColorNormalize(self.into()).into() }
    }

    /// Returns HSV values for a Color
    #[inline]
    pub fn color_to_hsv(&self) -> Vector3 {
        unsafe { crate::ffi::ColorToHSV(self.into()).into() }
    }

    /// Returns a Color from HSV values
    #[inline]
    pub fn color_from_hsv(hue: f32, saturation: f32, value: f32) -> Color {
        unsafe { crate::ffi::ColorFromHSV(hue, saturation, value).into() }
    }

    /// Returns color from normalized values [0..1]
    #[inline]
    pub fn color_from_normalized(normalized: Vector4) -> Color {
        unsafe { crate::ffi::ColorFromNormalized(normalized.into()).into() }
    }

    /// Returns a Color struct from hexadecimal value
    #[inline]
    pub fn get_color(hex_value: u32) -> Color {
        unsafe { crate::ffi::GetColor(hex_value).into() }
    }

    /// Get color multiplied with another color
    #[inline]
    pub fn tint(&self, color: Self) -> Self {
        unsafe { crate::ffi::ColorTint(self.into(), color.into()).into() }
    }
    /// Get color with brightness correction, brightness factor goes from -1.0f to 1.0f
    #[inline]
    pub fn brightness(&self, factor: f32) -> Self {
        unsafe { crate::ffi::ColorBrightness(self.into(), factor).into() }
    }
    /// Get color with contrast correction, contrast values between -1.0f and 1.0f
    #[inline]
    pub fn contrast(&self, factor: f32) -> Self {
        unsafe { crate::ffi::ColorContrast(self.into(), factor).into() }
    }
    /// Get color with alpha applied, alpha goes from 0.0f to 1.0f
    #[inline]
    pub fn alpha(&self, alpha: f32) -> Self {
        unsafe { crate::ffi::ColorAlpha(self.into(), alpha).into() }
    }

    /// Get color with alpha applied, alpha goes from 0.0f to 1.0f
    #[deprecated = "Use Color::alpha instead"]
    pub fn fade(&self, alpha: f32) -> Self {
        unsafe { crate::ffi::Fade(self.into(), alpha).into() }
    }

    /// Color fade-in or fade-out, alpha goes from 0.0f to 1.0f
    #[inline]
    pub fn color_alpha_blend(dst: &Color, src: &Color, tint: &Color) -> Color {
        unsafe { crate::ffi::ColorAlphaBlend(dst.into(), src.into(), tint.into()).into() }
    }
    // TODO: / Check if color is equal to another.
    // #[inline]
    // pub fn is_equal(&self, rhs: impl Into<crate::ffi::Color>) -> bool {
    //     unsafe { crate::ffi::ColorIsEqual(self.into(), rhs.into()) }
    // }

    /// Get color lerp interpolation between two colors, factor [0.0f..1.0f]
    #[inline]
    pub fn lerp(&self, rhs: Color, factor: f32) -> Color {
        unsafe { crate::ffi::ColorLerp(self.into(), rhs.into(), factor).into() }
    }
}

// TODO: / NOTE(IOI_XD): We manually implement PartialEq as of 5.5 to use Raylib's function. It's very unlikely it will ever
